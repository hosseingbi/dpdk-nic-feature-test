...
hn_test_qinxq::hn_test_qinxq(u_int32_t lcore_id)
    :hn_test(lcore_id)
{
}

hn_test_qinxq::~hn_test_qinxq()
{}

int hn_test_qinxq::get_burst_pkts(__rte_unused rte_mbuf **m, __rte_unused u_int32_t max_burst_size, u_int32_t &ret_burst_size, __rte_unused rte_mempool *mempool) 
{
    ret_burst_size = 0;
    return 1;
}

int hn_test_qinxq::process_rx_burst_pkts(__rte_unused rte_mbuf **m , u_int32_t size, u_int32_t queue_id) 
{
...
void hn_test_qinxq::update_nic_global_config(hn_driver *nic_driver, u_int16_t port_id, rte_eth_conf &port_conf) 
{
    nic_driver->set_rss_qinxq_config(port_id, port_conf);
    // nic_driver->set_rss_config(port_id, port_conf);
}

void hn_test_qinxq::update_nic_after_start(hn_driver *nic_driver, u_int16_t port_id, u_int32_t nb_queues) 
{
    nic_driver[port_id].set_rss_qinxq_rte_flow_config(port_id, nb_queues, 0);
...
void hn_test_qinxq::show_the_test_results() 
{
}

void hn_test_qinxq::before_receiving(u_int16_t port_id)
{
    memset(&retaconf, 0, sizeof(xena_reta_conf));
    port_rss_reta_query(port_id, (xena_reta_conf_ptr)&retaconf);
}

void hn_test_result_qinxq::show_test_results()
{
}
